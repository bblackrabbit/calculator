class Calculator {

    getPolynomial(str) {
        if (str instanceof Array) return new Polynomial(str);
        if (str && typeof str == 'string') {
            str = str.replaceAll(' ', '').replaceAll('x', '').replaceAll('*', '')
                .replaceAll('(', '').replaceAll(')', '');
            const arr = str.split('+');
            const members = [];
            arr.forEach(elem => {
                const arr1 = elem.split('^');
                members.push(new Member(arr1[0] - 0, arr1[1] - 0));
            })
            return new Polynomial(members);
        }
        return null;
    }

    getMatrix(str) {
        if (str instanceof Array) return new Matrix(str);
        if (str && typeof str === 'string') {
            const arr = str.split('\n');
            const values = [];
            for (let i = 0; i < arr.length; i++) {
                values.push(arr[i].split(',').map(el => this.getValue(el)));
            }
            if (values[0] instanceof Array) {
                return new Matrix(values);
            }
        }
        return null;
    }

    getComplex(str) {
        if (typeof str === "number") return new Complex(str);
        if (str && typeof str === 'string') {
            const arrStr = str.split('i*');
            if (arrStr.length === 2) {
                if (arrStr[0].includes('+')) {
                    arrStr[0] = arrStr[0].replace('+', '');
                    return new Complex(arrStr[0] - 0, arrStr[1] - 0);
                }
                if (arrStr[0].includes('-')) {
                    arrStr[0] = arrStr[0].replace('-', '');
                    return new Complex(arrStr[0] - 0, arrStr[1] - 0);
                }
            }
            if (arrStr.length === 1) {
                if (isNaN(arrStr[0] - 0)) return null;
                return new Complex(arrStr[0] - 0);
            }
        }
    }

    getVector(str) {
        if (str instanceof Array) return new Vector(str);
        if (str && typeof str == 'string') {
            const arr = str.replace('(', '').replace(')', '')
                .split(' ').map(el => this.getValue(el));
            return new Vector(arr);
        }
        return null;
    }

    getValue(str) {
        if (str.includes('\n')) return this.getMatrix(str);
        if (str.includes('(')) return this.getVector(str);
        if (str.includes('i')) return this.getComplex(str);
        if (str.includes('^')) return this.getPolynomial(str);
        return str - 0;
    }

    Complex(re, im) {
        return new Complex(re, im);
    }

    Vector(values) {
        return new Vector(values)
    }

    Matrix(values) {
        return new Matrix(values)
    }

    get(el) {
        if (el instanceof Matrix) return new MatrixCalculator(this.get(el.values[0][0]));
        if (el instanceof Vector) return new VectorCalculator(this.get(el.values[0]));
        if (el instanceof Complex) return new ComplexCalculator;
        if (el instanceof Polynomial) return new PolynomialCalculator;
        return new RealCalculator;
    }

    add(a, b) {
        return this.get(a).add(a, b)
    }

    sub(a, b) {
        return this.get(a).sub(a, b)
    }

    mult(a, b) {
        return this.get(a).mult(a, b)
    }

    div(a, b) {
        return this.get(a).div(a, b)
    }

    pow(a, p) {
        return this.get(a).pow(a, p)
    }

    prod(a, p) {
        return this.get(a).prod(a, p)
    }



    zero(type, el) {
        type = type ? type : el ? el.constructor.name : null;
        switch (type) {
            case 'Complex': return this.get(this.Complex()).zero();
            case 'Vector': return this.get(this.Vector()).zero(el.values.length);
            case 'Matrix': return this.get(this.Matrix()).zero(el.values.length);
            case 'default': return this.get().zero();
        }
    }

    one(type, el) {
        type = type ? type : el ? el.constructor.name : null;
        switch (type) {
            case 'Complex': return this.get(this.Complex()).one();
            case 'Vector': return this.get(this.Vector()).one(el.values.length);
            case 'Matrix': return this.get(this.Matrix()).one(el.values.length);
            case 'default': return this.get().one();
        }
    }



}